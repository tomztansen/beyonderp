...
public class DynamicDataService {
    ...
    public boolean isAuditActiveForAction(String actionType) {
        if (!auditGlobalEnabled)
            return false;
        if (actionType == null)
            return true;
        String upper = actionType.trim().toUpperCase();
        if ("INSERT".equals(upper))
            return auditInsertEnabled;
        if ("UPDATE".equals(upper))
            return auditUpdateEnabled;
    ...
    public void logAuditTrail(String tableName, String recordId, String actionType, Map<String, Object> oldData,
            Map<String, Object> newData) {
        if (!isAuditActiveForAction(actionType))
            return;
        if (tableName == null || tableName.equalsIgnoreCase("sys_audit_log"))
            return;
        try {
                    ...
                    if (obj instanceof com.vaadinerp.security.entity.AppUser) {
                        ...
                        if (user.getUsername() != null && !user.getUsername().trim().isEmpty()) {
                            ...
                            if (user.getFullName() != null && !user.getFullName().trim().isEmpty()
                                    && !user.getFullName().trim().equalsIgnoreCase(currentUser)) {
            ...
            jdbcTemplate.update(
                    "INSERT INTO sys_audit_log (table_name, record_id, action_type, action_by, action_dt, old_data_json, new_data_json) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, ?, ?)",
    ...
    public boolean restoreFromAuditLog(Long auditId) {
        try {
            ...
            // Cascade restore anak (detail / subform) jika ada yang terhapus bersamaan
            // dengan header ini
            try {
                Map<String, Object> logRow = jdbcTemplate.queryForMap("SELECT * FROM sys_audit_log WHERE id = ?",
                        auditId);
                    ...
                    if (headerPkVal != null && !tableName.equalsIgnoreCase("sys_audit_log")) {
                        ...
                        for (Map<String, Object> cLog : childLogs) {
                            ...
                            if (cOldJson != null && !cOldJson.trim().isEmpty()) {
                                Map<String, Object> cOldData = getObjectMapper().readValue(cOldJson,
                                        new com.fasterxml.jackson.core.type.TypeReference<Map<String, Object>>() {
                                        });
    ...
    private String resolvePrimaryKeyColumn(String tableName, Set<String> validCols, Map<String, Object> oldData) {
        ...
        if (rawName != null) {
            ...
            if ("meta_form".equals(lowerName) && validCols.contains("form_code"))
                return "form_code";
            if ("meta_lov".equals(lowerName) && validCols.contains("lov_code"))
                return "lov_code";
            if ("meta_report".equals(lowerName) && validCols.contains("report_code"))
                return "report_code";
            if ("meta_form_action".equals(lowerName) && validCols.contains("id"))
                return "id";
            if ("meta_field".equals(lowerName) && validCols.contains("id"))
                return "id";
        }

        if (validCols.contains("id"))
            return "id";
        if (validCols.contains("code"))
            return "code";
        if (validCols.contains("form_code"))
            return "form_code";
        if (validCols.contains("lov_code"))
            return "lov_code";
        if (validCols.contains("report_code"))
            return "report_code";

        if (oldData.containsKey("id"))
            return "id";
        if (oldData.containsKey("code"))
            return "code";
    ...
    /**
     * Validasi keamanan query SQL read-only dinamis untuk mencegah SQL Injection,
     * stacked queries, dan DoS.
     */
    public static String validateAndSanitizeSelectQuery(String sql) {
        ...
        boolean startsWithSelect = lower.startsWith("select ") || lower.startsWith("select\t")
        ...
        // 2. Tolak multi-statement / stacked query (semicolon di pertengahan atau
        // akhir)
        ...
        for (String token : prohibitedTokens) {
            if (padded.contains(" " + token.trim() + " ") || lower.contains(token.trim() + "(")) {
                throw new SecurityException("Akses ditolak: Perintah SQL mengandung kata kunci atau tabel terlarang ("
                        + token.trim() + ")!");
            }
        }
        return trimmed;
    ...
    /**
     * Validasi keamanan body PL/pgSQL trigger untuk mencegah injeksi kode
     * berbahaya.
    ...
    public static void validateTriggerBody(String body) {
        ...
        for (String token : prohibited) {
            if (normalized.contains(token)) {
                throw new SecurityException(
                        "Akses ditolak: Trigger body mengandung perintah terlarang (" + token.trim() + ")!");
            }
        }
    }

    /**
     * Validasi bahwa comparison operator hanya berisi operator SQL yang aman
     * (whitelist).
     */
    ...
    public String getQualifiedTableName(String tableName) {
        ...
        if ("global_master".equalsIgnoreCase(clean))
            return "dynamic.global_category";
        if (clean.contains("."))
            return clean;

        String lower = clean.toLowerCase();
        if (lower.startsWith("meta_") || lower.startsWith("sys_") || lower.equals("app_user")
                || lower.equals("app_role")) {
            return "public." + clean;
        }
    ...
    public String getLovQualifiedTableName(String tableName) {
        ...
        if ("global_master".equalsIgnoreCase(clean))
            return "dynamic.global_category";
        if (isCustomSelectQuery(clean))
            return validateAndSanitizeSelectQuery(clean);
        ...
        try {
            ...
            if (!res.isEmpty()) {
                for (Map<String, Object> row : res) {
                    ...
                    if (("CHARACTER VARYING".equals(dt) || "VARCHAR".equals(dt) || "CHAR".equals(dt)
                            || "CHARACTER".equals(dt)) && maxLen != null && !"null".equals(maxLen.toString())) {
                        row.put("formatted_type", "VARCHAR(" + maxLen + ")");
                    } else if ("NUMERIC".equals(dt) || "DECIMAL".equals(dt)) {
                        if (prec != null && !"null".equals(prec.toString())) {
                            row.put("formatted_type", "DECIMAL(" + prec + ","
                                    + (scale != null && !"null".equals(scale.toString()) ? scale : "0") + ")");
                        } else {
            ...
            return jdbcTemplate.query(sql, rs -> {
                ...
                if (rs != null) {
                    ...
                    for (int i = 1; i <= rsmd.getColumnCount(); i++) {
                        ...
                        String typeName = rsmd.getColumnTypeName(i) != null ? rsmd.getColumnTypeName(i).toLowerCase()
                                : "text";
                        if (typeName.contains("int") || typeName.contains("serial") || typeName.contains("numeric")
                                || typeName.contains("decimal") || typeName.contains("float")
                        ...
                        col.put("is_nullable",
                                rsmd.isNullable(i) == java.sql.ResultSetMetaData.columnNoNulls ? "NO" : "YES");
    ...
    public String fetchTriggerBody(String tableName, String triggerName) {
        if (tableName == null || triggerName == null)
            return "";
        String rawName = tableName.contains(".") ? tableName.substring(tableName.indexOf(".") + 1) : tableName;
    ...
    public void addOrUpdateTableTrigger(String tableName, TriggerDefinition trigger) {
        ...
        if (!isCurrentUserSuperAdmin()) {
            throw new SecurityException(
                    "Akses ditolak: Hanya SUPER_ADMIN yang boleh membuat atau mengubah database trigger!");
        }
    ...
    @Transactional
    public void addTableConstraint(String tableName, String constraintName, String type, String col, String refTable,
            String refCol, String expr) {
        validateSqlIdentifier(constraintName, "constraint name");
        validateSqlIdentifier(col, "column name");
        String qTableName = getQualifiedTableName(tableName);
        StringBuilder sql = new StringBuilder("ALTER TABLE ").append(qTableName).append(" ADD CONSTRAINT ")
        ...
        if ("FOREIGN KEY".equalsIgnoreCase(type)) {
            ...
            sql.append("FOREIGN KEY (").append(col).append(") REFERENCES ").append(qRefTable).append(" (")
                    .append(refCol).append(") ON DELETE CASCADE");
    ...
    @Transactional
    public void addTableColumn(String tableName, String columnName, String dataType, boolean nullable,
            String defaultVal) {
    ...
    @Transactional
    public void alterTableColumn(String tableName, String oldColName, String newColName, String newType,
            boolean nullable, String defaultVal) {
        ...
        if (newType != null && !newType.trim().isEmpty()) {
            jdbcTemplate.execute("ALTER TABLE " + qTable + " ALTER COLUMN " + targetCol + " TYPE " + newType.trim()
                    + " USING " + targetCol + "::" + newType.trim());
        }
        ...
        if (defaultVal != null && !defaultVal.trim().isEmpty()) {
            jdbcTemplate.execute(
                    "ALTER TABLE " + qTable + " ALTER COLUMN " + targetCol + " SET DEFAULT " + defaultVal.trim());
        } else {
    ...
    @Transactional
    public void updateTableConstraint(String tableName, String oldConstraintName, String newConstraintName, String type,
            String col, String refTable, String refCol, String expr) {
        ...
        if (formOpt.isPresent()) {
            ...
            if (form.getFields() != null && !form.getFields().isEmpty()) {
                ...
                for (FieldMeta field : form.getFields()) {
                    ...
                    if (fname.contains("name") || fname.contains("title") || fname.contains("desc")
                            || fname.contains("label")) {
                        bestField = field;
                        break;
                    }
                }
    ...
    private int executeAndLogSql(String sql, List<Object> args) {
        ...
        if (args != null) {
            List<String> types = args.stream()
                    .map(o -> o == null ? "NULL" : o.getClass().getSimpleName() + "(" + o + ")").toList();
    ...
    public void saveData(FormMeta formMeta, Map<String, Object> rawData, String fkColumn, Object fkValue) {
        ...
        if (fkColumn != null)
            validateSqlIdentifier(fkColumn, "foreign key column");
        ...
        if (formMeta.getTableName() != null && !formMeta.getTableName().trim().isEmpty()) {
            if (isUpdate) {
                ...
                try {
                    String selectSql = "SELECT * FROM " + getQualifiedTableName(formMeta.getTableName())
                            + " WHERE CAST(" + pk + " AS text) = ?";
                    List<Map<String, Object>> rows = jdbcTemplate.queryForList(selectSql,
                            masterId != null ? masterId.toString().trim() : "");
                    if (!rows.isEmpty()) {
                        oldRecord = rows.get(0);
                    }
                } catch (Exception ex) {
                    log.warn("Gagal mengambil record lama untuk audit (tabel: {}, pk: {}): {}", formMeta.getTableName(),
                            masterId, ex.getMessage());
                }
                ...
                for (FieldMeta field : formMeta.getFields()) {
                    ...
                    if (fieldName.equalsIgnoreCase("inputby") || fieldName.equalsIgnoreCase("inputdt") ||
                        ...
                        continue;
                    if (!field.isSaveOnUpdate())
                        continue;
                    if ("SUBFORM_GRID".equalsIgnoreCase(field.getComponentType()))
                        continue; // Skip subform grid columns
                ...
                if (!args.isEmpty()) {
                    ...
                    String sql = "UPDATE " + getQualifiedTableName(formMeta.getTableName()) + " SET "
                            + setClause.toString() + whereClause.toString();
                    ...
                    for (FieldMeta field : formMeta.getFields()) {
                        if (field.isAuditLog()) {
                            ...
                            if (oldStr == null || !oldStr.equals(newStr)) {
                                auditBatch.add(new Object[] { formMeta.getFormCode(), formMeta.getTableName(),
                                        masterId != null ? masterId.toString() : "", "UPDATE", fieldName, oldStr,
                                        newStr, currentUser, nowDt });
                            }
                        }
                    }
                ...
                if (formMeta.getFields() != null) {
                    for (FieldMeta field : formMeta.getFields()) {
                        if (field.getSequenceCode() != null && !field.getSequenceCode().trim().isEmpty()) {
                            ...
                            if (val == null || val.toString().trim().isEmpty() || val.toString().startsWith("[AUTO")
                ...
                for (FieldMeta field : formMeta.getFields()) {
                    ...
                    if (fieldName.equalsIgnoreCase(pk)) {
                        ...
                        if ((field.getSequenceCode() == null || field.getSequenceCode().trim().isEmpty()) &&
                                (pkVal == null || pkVal.toString().trim().isEmpty()
                                        || "0".equals(pkVal.toString().trim())
                                        || pkVal.toString().trim().startsWith("[AUTO")
                                        || pkVal.toString().trim().startsWith("⚡"))) {
                            continue;
                        }
                    }
                    if (fieldName.equalsIgnoreCase("inputby") || fieldName.equalsIgnoreCase("inputdt") ||
                            fieldName.equalsIgnoreCase("updateby") || fieldName.equalsIgnoreCase("updatedt"))
                        continue;
                    if (!field.isSaveOnInsert()
                            && (field.getSequenceCode() == null || field.getSequenceCode().trim().isEmpty()))
                        continue;
                    if ("SUBFORM_GRID".equalsIgnoreCase(field.getComponentType()))
                        continue; // Skip subform grid columns
                    if (data.containsKey(fieldName)) {
                ...
                try {
                    ...
                    if (genId != null) {
                        ...
                        System.out.println(">> CAPTURED RETURNING ID FOR TABLE " + formMeta.getTableName() + " -> PK: "
                                + pk + " = " + masterId);
                    }
                } catch (Exception ex) {
                    log.warn("RETURNING clause gagal untuk tabel '{}', mencoba GeneratedKeyHolder/executeAndLogSql: {}",
                            formMeta.getTableName(), ex.getMessage());
                    org.springframework.jdbc.support.GeneratedKeyHolder keyHolder = new org.springframework.jdbc.support.GeneratedKeyHolder();
                    try {
                        jdbcTemplate.update(connection -> {
                            java.sql.PreparedStatement ps = connection.prepareStatement(sql,
                                    new String[] { finalMasterPk });
                        ...
                        if (keyHolder.getKey() != null) {
                            ...
                            System.out.println(">> CAPTURED KEYHOLDER ID FOR TABLE " + formMeta.getTableName()
                                    + " -> PK: " + pk + " = " + masterId);
                        }
                    } catch (Exception ex2) {
                        executeAndLogSql(sql, args);
                        try {
                            Object lastId = jdbcTemplate.queryForObject("SELECT " + finalMasterPk + " FROM "
                                    + qMasterTable + " ORDER BY " + finalMasterPk + " DESC LIMIT 1", Object.class);
                            if (lastId != null) {
                                masterId = lastId;
                                data.put(pk, masterId);
                                data.put("id", masterId);
                                ...
                                System.out.println(">> CAPTURED LAST ID FOR TABLE " + formMeta.getTableName()
                                        + " -> PK: " + pk + " = " + masterId);
                            }
                        } catch (Exception ignored) {
                        }
                ...
                if (formMeta.getFields() != null) {
                    ...
                    for (FieldMeta field : formMeta.getFields()) {
                        if (field.isAuditLog()) {
                            ...
                            if (newStr != null) {
                                auditBatch.add(new Object[] { formMeta.getFormCode(), formMeta.getTableName(),
                                        masterId != null ? masterId.toString() : "", "INSERT", fieldName, null, newStr,
                                        currentUser, nowDt });
                            }
                        }
                    }
        ...
        if (formMeta.getFields() != null) {
            for (FieldMeta field : formMeta.getFields()) {
                if ("SUBFORM_GRID".equalsIgnoreCase(field.getComponentType())) {
                    ...
                    if (childFormCode == null || childFormCode.trim().isEmpty())
                        continue;

                    FormMeta childFormMeta = formMetaRepository.findById(childFormCode).orElse(null);
                    if (childFormMeta == null)
                        continue;

                    // 1. Delete removed rows
                    if (data.containsKey(subformFieldName + "_deleted")
                            && data.get(subformFieldName + "_deleted") instanceof List) {
                        @SuppressWarnings("unchecked")
                        List<Map<String, Object>> deletedRows = (List<Map<String, Object>>) data
                                .get(subformFieldName + "_deleted");
    ...
    public String generateNextSequence(String seqCode) {
        ...
        if (list.isEmpty()) {
            list = jdbcTemplate.queryForList("SELECT * FROM dynamic.md_sequence WHERE LOWER(seq_code) = LOWER(?)",
                    cleanCode);
            if (list.isEmpty()) {
        ...
        if ("MONTHLY".equalsIgnoreCase(resetPeriod)
                && (now.getYear() != lastDate.getYear() || now.getMonthValue() != lastDate.getMonthValue())) {
            needReset = true;
        ...
        try {
            jdbcTemplate.update(
                    "UPDATE dynamic.md_sequence SET current_val = ?, last_reset_date = ? WHERE seq_code = ?",
        ...
        String numberPart = (paddingLen > 0) ? String.format("%0" + paddingLen + "d", nextVal)
    ...
    public List<String> getActiveSequenceCodes() {
        try {
            return jdbcTemplate.queryForList("SELECT seq_code FROM dynamic.md_sequence WHERE status = 'Active'",
                    String.class);
        } catch (Exception e) {
    ...
    private Object sanitizeJdbcValue(String tableName, String columnName, Object val) {
        ...
        if (val instanceof Boolean bVal && tableName != null && columnName != null) {
            ...
            if ("character varying".equals(colType) || "varchar".equals(colType) || "text".equals(colType)
                    || "char".equals(colType)) {
            ...
            if (!trimmedStr.isEmpty()) {
                ...
                if ("integer".equals(colType) || "smallint".equals(colType) || "serial".equals(colType)
                    ...
                    try {
                        ...
                        log.trace("Konversi ke Long gagal untuk '{}': {}", trimmedStr, ex.getMessage());
                    }
                } else if ("numeric".equals(colType) || "decimal".equals(colType) || "real".equals(colType)
                        || "double precision".equals(colType)) {
                    try {
                        return new java.math.BigDecimal(trimmedStr);
                    } catch (Exception ex) {
                        log.trace("Konversi ke BigDecimal gagal untuk '{}': {}", trimmedStr, ex.getMessage());
                    }
                } else if ("date".equals(colType)) {
                    try {
                        return java.sql.Date.valueOf(trimmedStr);
                    } catch (Exception ex) {
                        try {
                            return java.time.LocalDate.parse(trimmedStr);
                        } catch (Exception ignored) {
                        }
                    }
                } else if (colType.startsWith("timestamp")) {
                    try {
                        return java.sql.Timestamp.valueOf(trimmedStr);
                    } catch (Exception ex) {
                        try {
                            String normalized = trimmedStr.replace("T", " ");
                            if (normalized.contains("+")
                                    || (normalized.contains("-") && normalized.lastIndexOf("-") > 10)) {
                                try {
                                    return java.sql.Timestamp
                                            .from(java.time.OffsetDateTime.parse(trimmedStr).toInstant());
                                } catch (Exception ignored) {
                                }
                            }
                    ...
                    return "true".equalsIgnoreCase(trimmedStr) || "1".equals(trimmedStr)
                            || "active".equalsIgnoreCase(trimmedStr);
                }
            }
        }
        return val;
    ...
    public void deleteData(FormMeta formMeta, Map<String, Object> rawData) {
        ...
        if (pkObj != null && !pkObj.toString().trim().isEmpty()) {
            ...
            try {
                Map<String, Object> queried = jdbcTemplate.queryForMap("SELECT * FROM "
                        + getQualifiedTableName(formMeta.getTableName()) + " WHERE CAST(" + pk + " AS text) = ?",
                        pkVal);
                if (queried != null)
                    oldRow.putAll(queried);
            } catch (Exception ignored) {
            }
            if (oldRow.isEmpty()) {
                oldRow.putAll(data);
            ...
            if (formMeta.getFields() != null && !oldRow.isEmpty()) {
                for (FieldMeta field : formMeta.getFields()) {
                    if (!field.isDetail() && field.isAuditLog()) {
                        ...
                        String oldStr = oldVal != null ? oldVal.toString() : null;
                        if (oldStr != null) {
                            auditBatch.add(new Object[] { formMeta.getFormCode(), formMeta.getTableName(), pkVal,
            ...
            boolean isMasterDetail = "MASTER_DETAIL".equalsIgnoreCase(formMeta.getFormType())
                    || (formMeta.getDetailTableName() != null && !formMeta.getDetailTableName().trim().isEmpty()
                            && formMeta.getDetailForeignKey() != null
                            && !formMeta.getDetailForeignKey().trim().isEmpty());

            if (isMasterDetail) {
                String detailTable = formMeta.getDetailTableName();
                String detailFk = formMeta.getDetailForeignKey();
                if (detailTable != null && !detailTable.trim().isEmpty() && detailFk != null
                        && !detailFk.trim().isEmpty()) {
                    try {
                        List<Map<String, Object>> dtlRows = jdbcTemplate.queryForList(
                                "SELECT * FROM " + getQualifiedTableName(detailTable) + " WHERE CAST(" + detailFk
                                        + " AS text) = ?",
                                pkVal);
                        String dtlPkCol = "id";
                        try {
                            ...
                            List<String> pkCols = jdbcTemplate.queryForList(pkColQuery, String.class,
                                    detailTable.toLowerCase());
                            if (!pkCols.isEmpty() && pkCols.get(0) != null)
                                dtlPkCol = pkCols.get(0);
                        } catch (Exception ignored) {
                        }
                        for (Map<String, Object> rawDRow : dtlRows) {
                            ...
                            if (rawDRow != null)
                                dRow.putAll(rawDRow);
                            String dPkVal = dRow.get(dtlPkCol) != null ? dRow.get(dtlPkCol).toString()
                                    : (dRow.get("id") != null ? dRow.get("id").toString() : pkVal);
                            ...
                            if (formMeta.getFields() != null) {
                                for (FieldMeta field : formMeta.getFields()) {
                                    if (field.isDetail() && field.isAuditLog()) {
                                        ...
                                        String oldStr = oldVal != null ? oldVal.toString() : null;
                                        if (oldStr != null) {
                                            auditBatch.add(new Object[] { formMeta.getFormCode(), detailTable, dPkVal,
                                                    "DELETE", fieldName, oldStr, null, currentUser, auditNowDt });
                                        }
                                    }
                                }
                        ...
                        log.warn("Gagal merekam audit trail detail sebelum delete (tabel: {}, fk: {}, pk: {}): {}",
                                detailTable, detailFk, pkVal, ex.getMessage());
                    }
                }
            }
                ...
                for (FieldMeta field : formMeta.getFields()) {
                    if ("SUBFORM_GRID".equalsIgnoreCase(field.getComponentType())) {
                        ...
                        if (childForm != null) {
                            ...
                            if (childTable != null && !childTable.trim().isEmpty() && childFk != null
                                    && !childFk.trim().isEmpty()) {
                                try {
                                    List<Map<String, Object>> subRows = jdbcTemplate.queryForList(
                                            "SELECT * FROM " + getQualifiedTableName(childTable) + " WHERE CAST("
                                                    + childFk + " AS text) = ?",
                                            pkVal);
                                    String subPkCol = "id";
                                    try {
                                        ...
                                        if (!pkCols.isEmpty() && pkCols.get(0) != null)
                                            subPkCol = pkCols.get(0);
                                    } catch (Exception ignored) {
                                    }
                                    for (Map<String, Object> rawSRow : subRows) {
                                        Map<String, Object> sRow = new java.util.TreeMap<>(
                                                String.CASE_INSENSITIVE_ORDER);
                                        if (rawSRow != null)
                                            sRow.putAll(rawSRow);
                                        String sPkVal = sRow.get(subPkCol) != null ? sRow.get(subPkCol).toString()
                                                : (sRow.get("id") != null ? sRow.get("id").toString() : pkVal);
                                        ...
                                        if (childForm.getFields() != null) {
                                            for (FieldMeta childField : childForm.getFields()) {
                                                if (childField.isAuditLog()) {
                                                    ...
                                                    String oldStr = oldVal != null ? oldVal.toString() : null;
                                                    if (oldStr != null) {
                                                        auditBatch.add(new Object[] { childForm.getFormCode(),
                                                                childTable, sPkVal, "DELETE", fieldName, oldStr, null,
                                                                currentUser, auditNowDt });
                                                    }
                                                }
                                                if (fileStorageService != null && ("FILE_UPLOAD"
                                                        .equalsIgnoreCase(childField.getComponentType())
                                                        || "IMAGE_UPLOAD"
                                                                .equalsIgnoreCase(childField.getComponentType()))) {
                                    ...
                                    try {
                                        ...
                                        List<String> cols = jdbcTemplate.queryForList(checkSubCol, String.class,
                                                childTable.toLowerCase());
                                    ...
                                    if (subHasIsDeleted) {
                                        ...
                                        java.sql.Timestamp delTs = java.sql.Timestamp
                                                .valueOf(java.time.LocalDateTime.now());
                                        jdbcTemplate.update("UPDATE " + getQualifiedTableName(childTable)
                                                + " SET is_deleted = true, delete_by = ?, delete_date = ? WHERE CAST("
                                                + childFk + " AS text) = ?", delUser, delTs, pkVal);
                                    } else {
                                        jdbcTemplate.update("DELETE FROM " + getQualifiedTableName(childTable)
                                                + " WHERE CAST(" + childFk + " AS text) = ?", pkVal);
                                    }
                                } catch (Exception ex) {
                                    log.warn("Gagal cascade delete subform (tabel: {}, fk: {}, pk: {}): {}", childTable,
                                            childFk, pkVal, ex.getMessage());
                                }
                            }
                        }
                    }
            ...
            if (fileStorageService != null && formMeta.getFields() != null && data != null) {
                for (FieldMeta field : formMeta.getFields()) {
                    if ("FILE_UPLOAD".equalsIgnoreCase(field.getComponentType())
                        ...
                        if (val == null && oldRow != null)
                            val = oldRow.get(field.getFieldName());
                        if (val != null && !val.toString().trim().isEmpty()) {
                            fileStorageService.deleteFilesByDelimitedString(val.toString());
                        }
                    }
            ...
            if (hasSoftDelete) {
                ...
                try {
                    for (Map<String, Object> c : fetchTableSchemaDetails(formMeta.getTableName())) {
                        String cn = ((String) c.get("column_name")).toLowerCase();
                        if (cn.equals("delete_by") || cn.equals("deleted_by"))
                            hasDelBy = true;
                        if (cn.equals("delete_dt") || cn.equals("delete_date") || cn.equals("deleted_at"))
                            hasDelDt = true;
                    }
                } catch (Exception ignored) {
                }
                ...
                StringBuilder sqlUpdate = new StringBuilder(
                        "UPDATE " + getQualifiedTableName(formMeta.getTableName()) + " SET " + sdCol + " = true");
                List<Object> args = new ArrayList<>();
                if (hasDelBy) {
                ...
                if (isMasterDetail) {
                    ...
                    if (detailTable != null && !detailTable.trim().isEmpty() && detailFk != null
                        ...
                        try {
                            if (dtlSoft) {
                                jdbcTemplate.update("UPDATE " + getQualifiedTableName(detailTable) + " SET " + dtlSdCol
                                        + " = true WHERE CAST(" + detailFk + " AS text) = ?", pkVal);
                            } else {
                                jdbcTemplate.update("DELETE FROM " + getQualifiedTableName(detailTable) + " WHERE CAST("
                                        + detailFk + " AS text) = ?", pkVal);
                            }
                        } catch (Exception ex) {
                            log.warn("Gagal cascade delete detail pada soft delete (tabel: {}): {}", detailTable,
                                    ex.getMessage());
                        }
                    }
                }
                                ...
                                if (childTable != null && !childTable.trim().isEmpty() && childFk != null
                                    ...
                                    try {
                                        for (Map<String, Object> c : fetchTableSchemaDetails(childTable)) {
                                            String cn = ((String) c.get("column_name")).toLowerCase();
                                            if (cn.equals("is_deleted") || cn.equals("is_delete")
                                                    || cn.equals("deleted")) {
                                                subSoft = true;
                                                subSdCol = cn;
                                                break;
                                    ...
                                    try {
                                        if (subSoft) {
                                            jdbcTemplate.update("UPDATE " + getQualifiedTableName(childTable) + " SET "
                                                    + subSdCol + " = true WHERE CAST(" + childFk + " AS text) = ?",
                                                    pkVal);
                                        } else {
                                            jdbcTemplate.update("DELETE FROM " + getQualifiedTableName(childTable)
                                                    + " WHERE CAST(" + childFk + " AS text) = ?", pkVal);
                                        ...
                                        log.warn("Gagal cascade delete subform pada soft delete (tabel: {}): {}",
                                                childTable, ex.getMessage());
                                    }
                                }
                            }
                        }
            ...
            if (isMasterDetail) {
                ...
                if (detailTable != null && !detailTable.trim().isEmpty() && detailFk != null
                        && !detailFk.trim().isEmpty()) {
                    try {
                        String sqlDtl = "DELETE FROM " + getQualifiedTableName(detailTable) + " WHERE CAST(" + detailFk
                                + " AS text) = ?";
                        jdbcTemplate.update(sqlDtl, pkVal);
                    } catch (Exception ex) {
                        log.warn("Gagal cascade delete detail (tabel: {}, fk: {}, pk: {}): {}", detailTable, detailFk,
                                pkVal, ex.getMessage());
                    }
                }
            }
                    ...
                    if ("SUBFORM_GRID".equalsIgnoreCase(field.getComponentType())) {
                        ...
                        if (childForm != null) {
                            ...
                            if (childTable != null && !childTable.trim().isEmpty() && childFk != null
                                    && !childFk.trim().isEmpty()) {
                                try {
                                    String sqlDtl = "DELETE FROM " + getQualifiedTableName(childTable) + " WHERE CAST("
                                            + childFk + " AS text) = ?";
                                    jdbcTemplate.update(sqlDtl, pkVal);
                                } catch (Exception ex) {
                                    log.warn("Gagal cascade delete subform (tabel: {}, fk: {}, pk: {}): {}", childTable,
    ...
    private void ensureTableExists(FormMeta formMeta) {
        ...
        for (FieldMeta field : formMeta.getFields()) {
            ...
            if (!field.isSaveOnInsert() && !field.isSaveOnUpdate())
                continue; // Skip transient/custom view fields
            ...
            if ("DECIMALBOX".equalsIgnoreCase(field.getComponentType())) {
                ...
                typeDef = "TIME";
            } else if ("FILE_UPLOAD".equalsIgnoreCase(field.getComponentType())
                    || "IMAGE_UPLOAD".equalsIgnoreCase(field.getComponentType())
                    || "TEXTAREA".equalsIgnoreCase(field.getComponentType())) {
                typeDef = "TEXT";
            } else {
                typeDef = "VARCHAR(255)";
            }
    ...
    public String getCurrentLoggedUser() {
        try {
            ...
            log.debug("Tidak dapat mengambil user dari VaadinSession (mungkin konteks background): {}",
                    ex.getMessage());
        }
        return "SYSTEM";
    }
    ...
    public void recordFieldAuditLog(String formCode, String tableName, Object recordId, String actionType,
            String fieldName, Object oldValue, Object newValue, String actionBy) {
        ...
        try {
            ...
            jdbcTemplate.update(sql, formCode, tableName, recordId != null ? recordId.toString() : "", actionType,
                    fieldName, oldStr, newStr, actionBy, java.sql.Timestamp.valueOf(java.time.LocalDateTime.now()));
        } catch (Exception ex) {
            log.warn("Gagal mencatat audit log (form: {}, tabel: {}, field: {}, recordId: {}): {}", formCode, tableName,
                    fieldName, recordId, ex.getMessage());
        }
    }
    ...
    public List<Map<String, Object>> fetchFieldAuditLogs(String filterFormCode, String filterTableName,
            String filterRecordId, String filterFieldName, String filterActionBy, int limit) {
        ensureFieldAuditTableExists();
    ...
    private Object getCaseInsensitiveValue(Map<String, Object> map, String key) {
        ...
        if (key.contains(".")) {
            ...
            if (scalarVal != null && !(scalarVal instanceof Map)) {
                if ("id".equalsIgnoreCase(parts[1]) || "value".equalsIgnoreCase(parts[1])
                        || parts[0].equalsIgnoreCase(parts[1])) {
                    return scalarVal;
                }
            }
        }
    ...
    public String getLovDefaultOrderByClause(String lovCodeOrTableName) {
        if (lovCodeOrTableName == null || lovCodeOrTableName.trim().isEmpty())
            return "";
            ...
            for (com.vaadinerp.components.FilterCondition condition : filters) {
                ...
                if (isNullOp || (condition.getValue() != null && !condition.getValue().toString().trim().isEmpty())) {
                    ...
                    if ("IS NULL".equals(compOp) || "IS NOT NULL".equals(compOp)) {
                        ...
                        if (("=".equals(compOp) || "!=".equals(compOp) || "<>".equals(compOp))
                                && (isBoolTrue || isBoolFalse)) {
    ...
    public String resolveExistingColumn(String tableName, String requestedCol) {
        if (tableName == null || requestedCol == null || requestedCol.isBlank())
            return requestedCol != null && !requestedCol.isBlank() ? requestedCol : "id";
        List<String> cols = getColumnsForQueryOrTable(tableName);
        if (cols.isEmpty())
            return requestedCol;
        for (String c : cols) {
            if (c.equalsIgnoreCase(requestedCol))
                return c;
        }
        for (String c : cols) {
            if (c.equalsIgnoreCase("id") || c.equalsIgnoreCase("code") || c.equalsIgnoreCase("kode")
                    || c.equalsIgnoreCase("no") || c.equalsIgnoreCase("nomor") || c.toLowerCase().endsWith("_id")
                    || c.toLowerCase().endsWith("_code") || c.toLowerCase().endsWith("_no")) {
                return c;
            }
        }
        return cols.get(0);
    ...
    public Map<String, Object> fetchLovRecord(String tableName, String valueColumn, Object value) {
        ...
        if (isCustomSelectQuery(trimmed)) {
            sql.append("SELECT * FROM ( ").append(validateAndSanitizeSelectQuery(trimmed))
                    .append(" ) AS subquery WHERE CAST(").append(valueColumn).append(" AS text) = ? LIMIT 1");
        } else {
            sql.append("SELECT * FROM ").append(getLovQualifiedTableName(trimmed)).append(" WHERE CAST(")
                    .append(valueColumn).append(" AS text) = ? LIMIT 1");
        }
    ...
    private void buildWhereClause(StringBuilder where, List<Object> args, Map<String, ?> filterValues,
            FormMeta formMeta) {
        ...
        for (Map.Entry<String, ?> entry : filterValues.entrySet()) {
            ...
            if (colName == null || criteriaObj == null)
                continue;
    ...
    private void appendConditionWithLov(StringBuilder where, List<Object> args, String colName, String op, String val,
        ...
        if (formMeta != null && formMeta.getFields() != null) {
            for (com.vaadinerp.meta.FieldMeta f : formMeta.getFields()) {
                if (colName.equalsIgnoreCase(f.getFieldName()) && f.getLovCode() != null
        ...
        String lblCol = lovMeta.getLabelColumn() != null && !lovMeta.getLabelColumn().isBlank()
                ...
                : valCol;
        String searchCol = lovMeta.getSearchColumn() != null && !lovMeta.getSearchColumn().isBlank()
                ? lovMeta.getSearchColumn().trim()
                : lblCol;
    ...
    public List<Map<String, Object>> fetchGridDataPaged(FormMeta formMeta, int offset, int limit,
            Map<String, ?> filterValues, String sortField, String sortDir) {
        if (formMeta == null)
            return new ArrayList<>();
        String baseFrom = getBaseFromClause(formMeta);
        StringBuilder where = new StringBuilder(" WHERE 1=1 ");
    ...
    public List<Map<String, Object>> fetchTableDataPaged(String tableName, int offset, int limit,
            Map<String, ?> filterValues, String sortField, String sortDir) {
        if (tableName == null || tableName.trim().isEmpty())
            return new ArrayList<>();
        String trimmed = tableName.trim();
    ...
    /**
     * Menyimpan urutan kolom baru hasil drag-and-drop dari Grid ke tabel
     * meta_field.
    ...
    public void saveColumnOrder(String formCode, List<String> orderedFieldNames) {
        ...
        // Penyesuaian nama tabel menjadi meta_field dan kolom col_order berdasarkan
        // FieldMeta.java
    ...
    public void saveUserGridOrder(String formCode, String gridId, List<String> orderedFieldNames) {
        if (formCode == null || gridId == null || orderedFieldNames == null || orderedFieldNames.isEmpty()
                || userGridPreferenceRepository == null || securityService == null) {
            return;
        }
        com.vaadinerp.security.entity.AppUser user = securityService.getCurrentUser();
    ...
    @Transactional
    public void createPhysicalTableAndTrigger(String tableName, List<ColumnDefinition> columns,
    ...
    public List<Map<String, Object>> fetchDetailTableData(String detailTableName, String fkColumn, Object fkValue) {
        ...
        String sql;
        if (isCustomSelectQuery(trimmed)) {
            sql = "SELECT * FROM ( " + validateAndSanitizeSelectQuery(trimmed) + " ) AS detail_sub WHERE CAST("
                    + fkColumn + " AS text) = ?" + orderBy;
        } else {
    ...
    public void saveMasterDetailData(FormMeta formMeta, Map<String, Object> rawMasterData,
        ...
        if (formMeta.getTableName() != null && !formMeta.getTableName().trim().isEmpty()) {
            if (isUpdate) {
                ...
                for (FieldMeta field : formMeta.getFields()) {
                    if (field.isDetail())
                        continue; // skip detail columns
                    String fieldName = field.getFieldName();
                    if (fieldName.equalsIgnoreCase(masterPk))
                        continue;
                    if (fieldName.equalsIgnoreCase("inputby") || fieldName.equalsIgnoreCase("inputdt") ||
                            fieldName.equalsIgnoreCase("updateby") || fieldName.equalsIgnoreCase("updatedt") ||
                            fieldName.equalsIgnoreCase("version"))
                ...
                if (formMeta.getFields() != null) {
                    for (FieldMeta field : formMeta.getFields()) {
                        if (!field.isDetail() && field.getSequenceCode() != null
                                && !field.getSequenceCode().trim().isEmpty()) {
                            String fieldName = field.getFieldName();
                ...
                try {
                    ...
                    log.warn(
                            "RETURNING clause gagal di saveMasterDetailData untuk tabel '{}', mencoba GeneratedKeyHolder: {}",
                            formMeta.getTableName(), ex.getMessage());
                    org.springframework.jdbc.support.GeneratedKeyHolder keyHolder = new org.springframework.jdbc.support.GeneratedKeyHolder();
                    jdbcTemplate.update(connection -> {
                        java.sql.PreparedStatement ps = connection.prepareStatement(sql,
                    ...
                    for (FieldMeta field : formMeta.getFields()) {
                        if (!field.isDetail() && field.isAuditLog()) {
                            ...
                            if (masterData.containsKey(fieldName)) {
                                ...
                                if (newStr != null) {
                                    auditBatch.add(new Object[] { formMeta.getFormCode(), formMeta.getTableName(),
                                            masterId != null ? masterId.toString() : "", "INSERT", fieldName, null,
                                            newStr, currentUser, auditNowDt });
                                }
                            }
                        }
            ...
            log.debug(
                    "Target table master untuk form '{}' kosong/null, melewati penyimpanan tabel master dan langsung memproses detail.",
                    formMeta.getFormCode());
        }
                ...
                if (dPkObj != null && !dPkObj.toString().trim().isEmpty()) {
                    ...
                    try {
                        Map<String, Object> q = jdbcTemplate.queryForMap(
                                "SELECT * FROM " + qDetailTable + " WHERE CAST(" + detailPk + " AS text) = ?", delId);
                        if (q != null)
                            oldDetail.putAll(q);
                    } catch (Exception ignored) {
                    }
                    if (oldDetail.isEmpty())
                        oldDetail.putAll(row);
                    ...
                    if (formMeta.getFields() != null) {
                        for (FieldMeta field : formMeta.getFields()) {
                            if (field.isDetail() && field.isAuditLog()) {
                                ...
                                String oldStr = oldVal != null ? oldVal.toString() : null;
                                if (oldStr != null) {
                                    auditBatch.add(new Object[] { formMeta.getFormCode(), detailTableName, delId,
                                            "DELETE", fieldName, oldStr, null, currentUser, auditNowDt });
                                }
                            }
                        }
        ...
        if (detailsData != null) {
            ...
            for (Map<String, Object> rawRow : detailsData) {
                ...
                boolean isTempRow = row.containsKey("_tempId") && row.get("_tempId") != null
                        && !row.get("_tempId").toString().trim().isEmpty();
                ...
                if (isDetailUpdate) {
                    ...
                    try {
                        String selectSql = "SELECT * FROM " + qDetailTable + " WHERE CAST(" + detailPk
                                + " AS text) = ?";
                        List<Map<String, Object>> rows = jdbcTemplate.queryForList(selectSql,
                                detailId != null ? detailId.toString().trim() : "");
                        if (!rows.isEmpty()) {
                    ...
                    if (oldDetailRecord != null) {
                        for (FieldMeta field : formMeta.getFields()) {
                            if (field.isDetail() && field.isAuditLog()) {
                                ...
                                String newStr = newVal != null ? newVal.toString() : null;
                                if (oldStr == null || !oldStr.equals(newStr)) {
                                    auditBatch.add(new Object[] { formMeta.getFormCode(), detailTableName,
                                            detailId != null ? detailId.toString() : "", "UPDATE", fieldName, oldStr,
                                            newStr, currentUser, auditNowDt });
                                }
                            }
                        }
                        ...
                        for (FieldMeta field : formMeta.getFields()) {
                            if (field.isDetail() && field.getSequenceCode() != null
                                ...
                                if (val == null || val.toString().trim().isEmpty() || val.toString().startsWith("[AUTO")
                                    ...
                                    log.info("Auto-generated sequence '{}' for detail field '{}' -> {}",
                                            field.getSequenceCode(), fieldName, genNum);
                                }
                            }
                        }
                    ...
                    String sql = "INSERT INTO " + qDetailTable + " (" + columns.toString() + ") VALUES ("
                            + valuesParam.toString() + ")";
                    org.springframework.jdbc.support.GeneratedKeyHolder keyHolder = new org.springframework.jdbc.support.GeneratedKeyHolder();
                    jdbcTemplate.update(connection -> {
                    ...
                    for (FieldMeta field : formMeta.getFields()) {
                        if (field.isDetail() && field.isAuditLog()) {
                            ...
                            if (row.containsKey(fieldName)) {
                                ...
                                if (newStr != null) {
                                    auditBatch.add(new Object[] { formMeta.getFormCode(), detailTableName,
                                            insertedDetailId != null ? insertedDetailId.toString() : "", "INSERT",
                                            fieldName, null, newStr, currentUser, auditNowDt });
                                }
                            }
                        }
                    }
        ...
        if ("MASTER_DETAIL".equalsIgnoreCase(formMeta.getFormType())) {
            ...
            for (FieldMeta field : formMeta.getFields()) {
                if (!field.isDetail())
                    continue; // skip master fields
                if (!field.isSaveOnInsert() && !field.isSaveOnUpdate())
                    continue; // Skip transient columns for detail table
                String colName = field.getFieldName();
                ...
                String alterSql = "ALTER TABLE " + qDetailTable + " ADD COLUMN IF NOT EXISTS " + colName + " "
                        + typeDef;
                try {
                    jdbcTemplate.execute(alterSql);
                } catch (Exception ignored) {
    ...
    public List<com.vaadinerp.meta.FormActionMeta> getFormActions(String formCode, String targetScope) {
        if (formActionMetaRepository == null || formCode == null)
        ...
        List<com.vaadinerp.meta.FormActionMeta> directActions = formActionMetaRepository
                .findByFormMeta_FormCodeIgnoreCase(formCode);
        ...
        // 2. Then check if formMeta has assigned actions via extraToolbars catalog
        // selection
        ...
        if (formMeta != null && formMeta.getExtraToolbars() != null && !formMeta.getExtraToolbars().trim().isEmpty()) {
            ...
            for (String c : codes) {
                ...
                if (!cleanCode.isEmpty()) {
                    com.vaadinerp.meta.FormActionMeta act = formActionMetaRepository
                            .findByActionCodeIgnoreCase(cleanCode);
                    if (act != null) {
                        if (targetScope == null || act.getTargetScope() == null || act.getTargetScope().isBlank()
                                || targetScope.equalsIgnoreCase(act.getTargetScope())) {
                            if (result.stream().noneMatch(a -> (a.getId() != null && a.getId().equals(act.getId()))
                                    || (a.getActionCode() != null && a.getActionCode().equalsIgnoreCase(cleanCode)))) {
                                result.add(act);
                            }
                        }
                    }
    ...
    public String evaluateFilterMappingDiagnostic(String filterMapping, Map<String, Object> headerRecord,
        ...
        for (String pair : pairs) {
            ...
            if (kv.length == 2) {
                ...
                if ("IS NULL".equalsIgnoreCase(valSpec) || "'IS NULL'".equalsIgnoreCase(valSpec)
                        || "\"IS NULL\"".equalsIgnoreCase(valSpec) || "IS_NULL".equalsIgnoreCase(valSpec)
                        || "null".equalsIgnoreCase(valSpec) || "'null'".equalsIgnoreCase(valSpec)
                        || "\"null\"".equalsIgnoreCase(valSpec)) {
                    sb.append("• ").append(col).append(" ➔ FILTER: IS NULL (Hanya data yang kosong/null)\n");
                    continue;
                } else if ("IS NOT NULL".equalsIgnoreCase(valSpec) || "'IS NOT NULL'".equalsIgnoreCase(valSpec)
                        || "\"IS NOT NULL\"".equalsIgnoreCase(valSpec) || "IS_NOT_NULL".equalsIgnoreCase(valSpec)) {
                ...
                if (valSpec.startsWith("header.") || valSpec.startsWith("\"header.")) {
                    isHeader = true;
                    headerKey = valSpec.replaceAll("[\"']", "")
                            .substring(valSpec.indexOf("header.") + "header.".length()).trim();
                    if (headerRecord != null) {
                        paramVal = getCaseInsensitiveValue(headerRecord, headerKey);
                    }
                } else if (valSpec.startsWith("picked.") || valSpec.startsWith("\"picked.")) {
                    isPicked = true;
                    pickedKey = valSpec.replaceAll("[\"']", "")
                            .substring(valSpec.indexOf("picked.") + "picked.".length()).trim();
    ...
    public List<Map<String, Object>> fetchLovDataWithActionFilters(String sourceLovCode, String filterMapping,
            Map<String, Object> headerRecord, String searchTerm) {
        return fetchLovDataWithActionFilters(sourceLovCode, filterMapping, headerRecord, null, searchTerm);
    }

    public List<Map<String, Object>> fetchLovDataWithActionFilters(String sourceLovCode, String filterMapping,
            Map<String, Object> headerRecord, Map<String, Object> pickedRecord, String searchTerm) {
        if (sourceLovCode == null || sourceLovCode.trim().isEmpty())
            return new ArrayList<>();
        ...
        if (filterMapping != null && !filterMapping.trim().isEmpty()) {
            ...
            for (String pair : pairs) {
                ...
                if (kv.length == 2) {
                    ...
                    Object paramVal = null;
                    if (valSpec.startsWith("header.") || valSpec.startsWith("\"header.")) {
                        String headerKey = valSpec.replaceAll("[\"']", "")
                                .substring(valSpec.indexOf("header.") + "header.".length()).trim();
                        if (headerRecord != null) {
                            paramVal = getCaseInsensitiveValue(headerRecord, headerKey);
                        }
                    } else if (valSpec.startsWith("picked.") || valSpec.startsWith("\"picked.")) {
                        String pickedKey = valSpec.replaceAll("[\"']", "")
                                .substring(valSpec.indexOf("picked.") + "picked.".length()).trim();
                        if (pickedRecord != null) {
                            paramVal = getCaseInsensitiveValue(pickedRecord, pickedKey);
                        }
                    ...
                    if (paramVal != null) {
                        if (paramVal instanceof java.util.Collection<?> colVal) {
                            if (colVal.isEmpty()) {
                                ...
                                String placeholders = colVal.stream().map(v -> "?")
