...
public class FormBuilderView extends VerticalLayout {
    ...
    public FormBuilderView(FormMetaRepository formMetaRepository, LovMetaRepository lovMetaRepository,
        ...
        btnAutoGenerateFields.getStyle().set("margin-top", "10px");
        ...
        formMetaLayout.add(formCodeField, formTitleField, formTypeCombo, tableNameField, viewTableField, pkField,
                ...
                detailFkField, autoCreateDbCheckbox, btnAutoGenerateFields);
        formMetaLayout.setColspan(assignedActionsCombo, 2);
        formMetaLayout.setColspan(autoCreateDbCheckbox, 2);
        formMetaLayout.setColspan(btnAutoGenerateFields, 2);
    ...
    private void setupPropertiesListeners() {
        ...
        propLovCode.addValueChangeListener(e -> {
            if (selectedField != null) {
                if (e.getValue() == null && selectedField.lovCode == null)
                    return;
                if (e.getValue() != null && e.getValue().equals(selectedField.lovCode))
                    return;
                ...
                if (!"SUBFORM_GRID".equalsIgnoreCase(selectedField.componentType)) {
                    ...
                    propBtnFilters.setEnabled(
                            isSelection && selectedField.lovCode != null && !selectedField.lovCode.trim().isEmpty());
                }
                rebuildCanvas();
            }
        });
    ...
    private Component buildFieldCard(FieldMetaTemp temp, int sequence) {
        ...
        cardHeader.setJustifyContentMode(JustifyContentMode.START);
    ...
    private void openAutoGenerateDialog() {
        ...
        layoutSelect.setItems("Standar Form (2 Kolom / Berjejer)", "Standar Form (1 Kolom / Vertikal)", "Memanjang Horizontal (Inline / Grid-Like)");
        ...
        Span infoSpan = new Span("💡 Kolom yang sudah ada di kanvas tidak akan digenerate ulang (mencegah duplikasi).");
        infoSpan.getStyle().set("color", "#64748b").set("font-size", "0.85rem");

        layout.add(tableSelect, layoutSelect, chkIsDetail, infoSpan);
        dialog.add(layout);
        ...
        btnGenerate.addClickListener(e -> {
            ...
            for (Map<String, Object> col : schemaDetails) {
                ...
                if (isDetailMode) {
                    ...
                    temp.rowGroup = 1;
                } else if ("Standar Form (1 Kolom / Vertikal)".equals(layoutMode)) {
                    if (addedCount > 0 || colInGroup > 0) currentGroup++;
