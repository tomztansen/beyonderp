...
public class GenericFormView extends VerticalLayout implements HasUrlParameter<String> {
    ...
    private void buildToolbar(FormMeta formDef) {
        ...
        if (!auth.canAccessScreen && !auth.canAdd && !auth.canEdit) {
    ...
    private void refreshExtraToolbarButtons() {
        ...
        if (currentFormDef == null || dynamicDataService == null)
            return;
        List<com.vaadinerp.meta.FormActionMeta> masterActions = dynamicDataService.getFormActions(
                currentFormDef.getFormCode(),
                "MASTER_TOOLBAR");
    ...
    private void preparePrint(com.vaadinerp.meta.ReportMeta report) {
        ...
        if (grid != null && grid.getSelectedItems() != null)
            rows.addAll(grid.getSelectedItems());
        ...
        try {
            ...
            if (sec.getCurrentUser() != null)
                user = sec.getCurrentUser().getUsername();
        } catch (Exception ignored) {
            // tanpa user: parameter SYSTEM $CURRENT_USER tetap null, bukan alasan
            // membatalkan cetak
        }
        ...
        com.vaadinerp.components.SafeButton cancel = new com.vaadinerp.components.SafeButton("Cancel",
                ev -> paramDialog.close());
                ...
                for (FieldFilterMeta filter : field.getFilters()) {
                    ...
                    if ("FIELD".equalsIgnoreCase(filter.getSourceType())) {
                        ...
                        if (sourceComponent instanceof com.vaadin.flow.component.HasValue) {
                            ...
                            if (currentRecordData != null && lookupKey != null
                                    && currentRecordData.containsKey(lookupKey)) {
            ...
            if ("SUBFORM_GRID".equalsIgnoreCase(field.getComponentType())) {
                ...
                if (comp instanceof SubformGridField) {
                    ...
                    if (parentPkValue != null && field.getLovCode() != null && field.getFormula() != null) {
                        ...
                        if (childForm != null) {
                            String childTableName = (childForm.getViewTable() != null
                                    && !childForm.getViewTable().trim().isEmpty())
